...
export default function ConvertPage() {
  ...
  const handleConvertSelected = () => {
    ...
    if (ids.length > 0) void convertMany(ids, targetFormat)
  }

  const handleDownloadAll = () => {
    const done = images.filter((img) => img.status === 'done' && img.processedBlob)
    if (done.length > 0) void downloadAllAsZip(done, 'converted-images.zip')
  }
    ...
    <>
      <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-8 sm:px-6">
        <div className="mb-6 text-center">
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white sm:text-3xl">Image Converter</h1>
          <p className="mx-auto mt-2 max-w-xl text-sm text-slate-500 dark:text-slate-400 sm:text-base">
            Convert up to 100 images between JPG, PNG, WebP, and AVIF. Everything happens on your device.
          </p>
        ...
        {images.length > 0 && (
          <>
            <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-100">Convert to</h3>
              <div className="relative mt-2 max-w-xs">
                <select
                  value={targetFormat}
                  onChange={(e) => setTargetFormat(e.target.value as OutputFormat)}
                  className="h-11 w-full appearance-none rounded-lg border border-slate-200 bg-white pl-3 pr-9 text-sm font-medium text-slate-800 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
                >
              ...
              <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                Every selected image will be re-encoded as {FORMAT_LABELS[targetFormat]}. If your browser can't
                encode it, we'll fall back gracefully and let you know.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 pb-6 lg:grid-cols-2">
              {images.map((img) => (
                <ConvertCard
                  ...
                  onConvert={(id) => void processImage(id, targetFormat)}
      ...
      {images.length > 0 && (
        <DownloadBar
          ...
          actionLabel="Convert"
          actionInProgressLabel="Converting…"
          onAction={handleConvertSelected}
